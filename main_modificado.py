...
async def guardar_datos(datos: DatosTarea):
    tareas = leer_datos("tareas.json")
    nueva_tarea = datos.dict()
    nueva_tarea["id"] = len(tareas) + 1
    tareas.append(nueva_tarea)
    guardar_datos("tareas.json", tareas)
...
async def actualizar_tarea(id: int, datos: DatosTarea):
    tareas = leer_datos("tareas.json")
    for idx, tarea in enumerate(tareas):
        if tarea.get("id") == id:
            tareas[idx] = datos.dict()
            tareas[idx]["id"] = id
            guardar_datos("tareas.json", tareas)
            return JSONResponse(content={"mensaje": "Tarea actualizada correctamente"}, status_code=200)
